...
// import '../Sidebars/styles/Sidebar2.css';
import { db } from "../config/firebase.js";
//import upvote_pujo_data from "./data/upvote_pujo";
import { Link } from 'react-router-dom';
import UpvotePujoRow from '../Sidebars/UpvotePujoRow.js';
...
function NotifSidebar2({ user,lang }) {
        ...
        <>
            <div className="sidebar2">
                <div className="notification">
                    ...
                    {notifAll.length === 0 && <div style={{textAlign:"center"}}>No notifications yet.</div> }
                    {notifAll.length !== 0 &&  <ul className="notification-list">
                        {notifAll.length >= 4 ? notifAll.slice(0,4).map(Notification) 
                        : notifAll.map(Notification)}
                        
                        <li style={{textAlign:"center"}}>{notifAll.length >= 4 && <Link to="/notifications"><button className="see-all-notif-button">See All</button></Link>}</li>
